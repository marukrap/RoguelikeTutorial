...
namespace
{
	...
	constexpr int maxItemsPerRoom = 2;
...
void Dungeon::placeEntities(World& world, Entity& player)
{
	...
	for (const auto& room : m_rooms)
	{
		...
		const int numItems = randomInt(0, maxItemsPerRoom);
		...
		for (int i = 0; i < numItems; ++i)
		{
			...
			} while (world.getItem(pos));

			auto item = std::make_unique<Item>(world, ItemType::Potion, L"체력 물약", 816);
			item->setPosition(pos);
			item->setFunction([] (Actor& actor)
			{
				actor.recoverHp(4);
			});
